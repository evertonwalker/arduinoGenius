...
//Método criado para configurar a saída das portas tanto os leds, como os botões, usado no Setup. 
void iniciaPortas() {
...
//Método utilizado para iniciar o jogo, ou seja pegar a combinação das cores.
void iniciaJogo() {
  //Lê um valor aleatório através da corrente do ar, já que a porta 0 está configurada pra isso, e envia pra função random
   
  int jogo = analogRead(0);
  randomSeed(jogo);
  //Cria uma sequência de luzes através de outro método chamado sorteiaCor();
  for (int i = 0; i < TAMANHO_SEQUENCIA; i++) {
    sequenciaLuzes[i] = sorteiaCor();
  }
}

//Gera uma sequência de cores aleatórias de valores de 2 a 5, contando com o 5, já que são os 4 leds.
int sorteiaCor() {
...
//Verifica a rodada de acordo com os valors passado dentro do Switch case
